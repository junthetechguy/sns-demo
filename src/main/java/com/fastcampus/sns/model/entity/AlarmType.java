...
// 아니면 클라이언트에서도 관리할수도 있다. 가령 AlarmType이 A인 경우 이런식으로 띄워주세요, B인 경우 저렇게 띄워주세요라고 할 수도 있는데 이게 웹 앱인 경우에는 상관이 없지만
// 모바일 앱인 경우 버전이 업데이트 되어야(ex. yes24 업데이트) 그게 변할 수 있어서 이런식으로 변화가능성이 있는 것들은 서버 자체에서(DB에서나 클라이언트에서 말고 서버 그 자체에서) 관리해주자.