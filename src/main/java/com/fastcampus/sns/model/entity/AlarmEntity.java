...
@Table(name = "\"alarm\"", indexes = {
        @Index(name = "user_id_aaidx", columnList = "user_id")
})
@TypeDef(name = "jsonb", typeClass = JsonBinaryType.class) // opensource library에서 들어있는 JsonBinaryType.class가 jsonb type라고 정의해주자.
...
public class AlarmEntity {
    ...
    // 이처럼 알람의 경우 굉장히 변화될 수 있는 환경 많다. 왜냐하면 서비스적으로 생각해봤을때 이 alarm을 보냄으로서 user의 접속을 유도할 수 있으므로 굉장히 많은 종류의 알람이 생길 수 있고, 서비스 적으로도
    ...
    // alarm을 뭉쳐서 보여주기 위해선 이렇게 AlarmType을 따로 column을 구분하도록 하자.


    // 지금은 당장 "new comment"와 "new like"가 alarm text의 전부이지만
    // 실전 서비스에서는 그런 간단한 text가 아니라, 누가 comment를 달았는가, 누가 like를 달았는가를 보여주고,
    // 알람을 눌렀을 경우에 해당 post로 이동하게 된다. 따라서 이 alarm을 누가 발생시켰는가에 대한 정보와 이 alarm이 어떤 post에 발생이 되었는지에 대한 주체(post, 댓글, 스토리 등)가 필요하다.
    ...
    // 따라서 이 경우에는 이런식으로 json으로 argument를 설정해서 유연하게 대처하자.
