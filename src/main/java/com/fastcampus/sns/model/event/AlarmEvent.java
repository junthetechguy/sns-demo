...
public class AlarmEvent { // alarm을 save할때 필요했던 모든 정보들(AlarmEntity의 field)을 다 넣어준다.
    private Integer receiveUserId; // Kafka에서 Key로 사용할건데 AlarmEntity의 field에는 UserEntity로 설정이 되어 있지만 UserEntity를 그대로 넣을 수는 없으니 Integer로 알람을 받는(receiver)의 UserId를 넣어준다.
