...
public class RedisConfiguration {
    ...
    public RedisConnectionFactory redisConnectionFactory() {
        ...
        factory.afterPropertiesSet(); // Lettuce로 만든 factory를 초기화(initializing) 시키는 용도
        return factory;
    }

    // Caching할때 생각해야해봐야 하는건 첫번째로 데이터의 변경이 너무 많은 데이터는 캐싱을 해봤자 소용이 없다.
    // 캐싱은 DB에 있는 데이터를 접근의 Cost가 조금 더 적은 곳에 놔둬서 가지고 오기 때문에 만약 원본 데이터인 DB의 데이터가 너무 많이 변화하게 되면 캐싱도 당연히 변화를 해야 하므로
    ...
    public RedisTemplate<String, User> userRedisTemplate(RedisConnectionFactory redisConnectionFactory) { // Bean으로 위에서 retun된 factory를 그대로 가져오게 된다.
        ...
        redisTemplate.setValueSerializer(new Jackson2JsonRedisSerializer<>(User.class)); // value가 User type이므로 이 Object serializer로 넣어주자.
